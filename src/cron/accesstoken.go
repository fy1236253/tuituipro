...
func monitorToken() {
	for {
		...
		if t < 600 { //  即将过期
			token := util.GetTuiKeToken(c.AppID, c.AppSecret)
			if token == nil {
...
func CheckToken() {
	for {
		if g.GetTuiKeAccessToken() == "" {
			log.Println("[warn] access token not ready, wait", g.Config().TuiKe.AppID)
...
func CheckAuthorizerAccessToken() {
	for {
		...
		if t < 600 && exist == true {
			userInfo := util.RefreshAccessToken(user)
			util.CheckIsNil(userInfo)
