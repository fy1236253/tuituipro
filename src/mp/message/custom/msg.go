...
func NewText(wxid, toUser, content, kfAccount string) (text *Text) {
	text = &Text{
...
// 新建图片消息.
//  如果不指定客服则 kfAccount 留空.
func NewImage(wxid, toUser, mediaId, kfAccount string) (image *Image) {
	image = &Image{
		MessageHeader: MessageHeader{
			WxId:    wxid,
			ToUser:  toUser,
...
// 新建图文消息.
//  如果不指定客服则 kfAccount 留空.
func NewNews(wxid, toUser string, articles []Article, kfAccount string) (news *News) {
	news = &News{
		MessageHeader: MessageHeader{
			WxId:    wxid,
			ToUser:  toUser,
